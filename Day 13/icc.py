...
class ICC:
    def __init__(self, intcode):
        if isinstance(intcode, str):
            intcode = [int(val) for val in intcode.split(",")]

        self.intcode_raw = Memory(intcode)
        self.intcode = Memory(intcode)
        self.index = 0
        self.rel_index = 0

    def restart(self):
        self.intcode = Memory(self.intcode_raw)
        self.index = 0
        self.rel_index = 0

    def parse_instruction(self):
        vals = []
        modes, op = divmod(self.intcode[self.index], 100)
        for i in range(1, cmd_len[op] + 1):
            modes, m = divmod(modes, 10)
            if m == M_POS:
                vals.append(self.intcode[self.index + i])
            elif m == M_IMM:
                vals.append(self.index + i)
            elif m == M_REL:
                vals.append(self.rel_index + self.intcode[self.index + i])

        return op, vals, cmd_len[op]

    def run(self, inp=0):
        while self.index < len(self.intcode):
            opcode, vals, offset = self.parse_instruction()
            if opcode == C_ADD:
                v1, v2, v3 = vals
                self.intcode[v3] = self.intcode[v1] + self.intcode[v2]
            elif opcode == C_MUL:
                v1, v2, v3 = vals
                self.intcode[v3] = self.intcode[v1] * self.intcode[v2]
            elif opcode == C_INP:
                self.intcode[vals[0]] = inp
            elif opcode == C_OUT:
                self.index += 2
                return self.intcode[vals[0]]
            elif opcode == C_JMP_T:
                v1, v2 = vals
                self.index = self.intcode[v2] if self.intcode[v1] != 0 else self.index + 3
            elif opcode == C_JMP_F:
                v1, v2 = vals
                self.index = self.intcode[v2] if self.intcode[v1] == 0 else self.index + 3
            elif opcode == C_LESS:
                v1, v2, v3 = vals
                self.intcode[v3] = int(self.intcode[v1] < self.intcode[v2])
            elif opcode == C_EQU:
                v1, v2, v3 = vals
                self.intcode[v3] = int(self.intcode[v1] == self.intcode[v2])
            elif opcode == C_REL:
                self.rel_index += self.intcode[vals[0]]
            elif opcode == C_HALT:
                return None
...
if __name__ == '__main__':
    ...
    intcode = [int(v) for v in intcode.split(",")]
    icc = ICC(intcode)
    r = icc.run(1)
    print(r == 2745604242)
